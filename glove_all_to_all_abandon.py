...
def find_cd_train_closest(train_arr):
	...
	for itest, test_vec in enumerate(train_arr):
		cd = np.dot(train_arr, test_vec)
		cd_winners = np.argpartition(cd, -(k + 1))[-(k + 1):]
		...
		l_closest_l_i.append(cd_idx_sorted)
		l_closest_l_hd.append(hd_of_winners)
...
def improve_sel_mat(word_arr, test_arr, sel_mat, bin_db, l_closest_l_i, l_i_test_closest, l_closest_l_hd):
...
def main():
	...
	improve_sel_mat(nd_train_recs, nd_q_recs, sel_mat, nd_bin_db, l_closest_l_i, l_i_test_closest, l_closest_l_hd)
	return
